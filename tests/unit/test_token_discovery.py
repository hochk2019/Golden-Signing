...
def test_env_override_wins(tmp_path: Path) -> None:
    ...
    env = {ENV_PKCS11: str(dll), "SystemRoot": str(tmp_path / "nowindows")}
...
def test_env_multiple_paths(tmp_path: Path) -> None:
    ...
    a.write_bytes(b"MZ")
    b.write_bytes(b"MZ")
    env = {ENV_PKCS11: f"{a};{b}", "SystemRoot": str(tmp_path / "nowindows")}
...
def test_missing_env_path_filtered(tmp_path: Path) -> None:
    env = {
        ENV_PKCS11: str(tmp_path / "nope-pkcs11.dll"),
        "SystemRoot": str(tmp_path / "nowindows"),
...
def test_extra_roots_scan(tmp_path: Path) -> None:
    ...
    other.write_text("x")
    found = discover_pkcs11_libraries(
        env={"SystemRoot": str(tmp_path / "nowindows"), "ProgramFiles": str(tmp_path / "pf")},
